...
fracking_clean <- fracking %>% 
  ...
  # clean up dates ----
  mutate(job_start_date = str_remove(job_start_date, " AM")) %>%  # remove 'AM' from string from job_start_date column cells
  mutate(datetime_start = mdy_hms(job_start_date)) %>%  # create new column "datetime_start". Input job_start_date data w/out " AM". Convert from string to date (save to new col)
  mutate(year = year(datetime_start)) %>%  #create "year" column. Input "datetime_start" 

  # select relevant cols ----
  select(datetime_start, year, state_name, county_name, well_name, total_base_water_volume) %>% #selects only the columns mentioned
