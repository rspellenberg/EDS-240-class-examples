...
jobs_clean %>% 
  ...
  # takes the top 10 highest paying occupations
  slice_max(order_by = total_earnings, n = 10) %>% 
  
    
  # reorganzing so highest earner is on top, descending order 
  ggplot(aes(x = fct_reorder(occupation, total_earnings), 
             y = total_earnings)) +
  
  geom_col() +
  
  # adding exact values and positioning them. scales::dollar puts $
  geom_text(aes(label = scales::dollar(total_earnings)), hjust = 1.1, color = "white") +
  
  # adding commas and dollar signs. label_currency only works within scale function
...
jobs_clean %>% 
  ...
  # adding exact values and positioning them. scales::dollar puts $
  geom_text(aes(label = scales::dollar(total_earnings)), hjust = -0.1, color = "darkblue") +
  
  # adding commas and dollar signs
  scale_y_continuous(labels = scales::label_currency(accuracy = 1, scale = 0.001, suffix = "k"), limits = c(0, 230000)) +
...
jobs_clean %>% 
  ...
  # can also use position = position_dodge()
...
jobs_clean %>% 
  ...
  #have to tell it starting and ending points of that line
...
ggplot(subset_jobs) +
  ...
  geom_point(aes(x = total_earnings_female, y = occupation),
    ...
    # adding commas and dollar signs
  scale_x_continuous(labels = scales::label_currency(accuracy = 1, scale = 0.001, suffix = "k")) +
